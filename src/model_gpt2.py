...
class PromptGPT2forCRS(GPT2PreTrainedModel):
    ...
    def set_diffusion_model(self, args, model, batch):
        # ***********************************************************************
        import diffusion_model.gaussian_diffusion as gd
        # diffusion model
        ...
        diffusion = gd.GaussianDiffusion(mean_type, 'linear-var', 0.1, 0.0001, 0.02, 100, 'cuda')
        diffusion_predict = diffusion.p_sample(model, batch, 0, False)

        return diffusion_predict
        # ***********************************************************************
    ...
    def forward(
        ...
        rec=False,
        entity_embeds=None,
        movie_entity_embeds=None,
        rec_labels=None,
        movie_rec_labels=None,
        conv=False,
        conv_labels=None,
        return_dict=True,
    ):
        ...
        if rec:
            ...
            #     rec_loss = F.cross_entropy(rec_logits, rec_labels)
                # 只取batc在·h_size范围内的隐藏状态（用户表示），将其与总的实体嵌入计算分数

            # 原始unicrs内容，正常跑
            # rec_logits = hidden_states[range(batch_size), sequence_lengths]  # (bs, hidden_size)
            # if movie_entity_embeds is None or movie_rec_labels is None:
            #     rec_logits @= entity_embeds.T  # entity_embeds.T  # (bs, n_item)
            #     if rec_labels is not None:
            #         # loss_fct = CrossEntropyLoss()
            #         rec_loss = F.cross_entropy(rec_logits, rec_labels)
            # else:
            #     rec_logits @= movie_entity_embeds.T.cuda()
            #     rec_loss = F.cross_entropy(rec_logits, movie_rec_labels)
            ...
            if movie_entity_embeds is None or movie_rec_labels is None:
                ...
                # # ***************************************************************
                # # diffusion func
                # from diffusion_model.args_model import parse_args
                # device, args = parse_args()
                # diffusion, dnn = denoise_out(args, device)
                # diffusion_predict = self.set_diffusion_model(args, dnn, rec_logits)
                # # ***************************************************************
