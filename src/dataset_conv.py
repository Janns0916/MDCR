...
class CRSConvDataCollator:
    def __init__(
        ...
        self.key2index = json.load(open('./data/conceptnet/key2index_3rd.json', encoding='utf-8'))
    ...
    def __call__(self, data_batch):
        ...
        if self.gen:
            ...
            for data in data_batch:
                ...
                keyword_batch.append([self.key2index.get(word.lower()) for word in data['keyword']])
        else:
        ...
        keyword_batch = padded_tensor(
            keyword_batch, pad_idx=0, pad_tail=True, device=self.device,
            use_amp=self.use_amp, debug=self.debug, max_len=self.entity_max_length
        )
        input_batch['keyword'] = keyword_batch
